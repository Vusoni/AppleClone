...
import {Color, SRGBColorSpace} from 'three' // To work with colors 

export default function MacbookModel14(props) {
  // Destructure color to display (Zustand State)
    const { color } = useAppleStore();
  const { nodes, materials, scene } = useGLTF('/models/macbook-14-transformed.glb');

  const texture = useTexture('/screen.png'); // Add Texture
    texture.colorSpace = SRGBColorSpace;
    texture.needsUpdate = true;


    // Use Effect to apply color in all meshes
  useEffect(() => {
    scene.traverse((child) => {
        if(child.isMesh) {
          // Color adjustment only if the part name is NOT notChangeParts
